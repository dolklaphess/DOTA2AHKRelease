...
使用传统键位或者特定英雄有不同改键的，请按以下格式修改脚本自动运行段中加入快捷键数组：hero:=new DotaClass(n_ability,dir,,["q","w","e","r","d","f"],["q","w","e","r","d","f"])。基于假腿的可以在Config.ahk中加入变量global heroname_ability_triggerkey和global ability_keyup。
