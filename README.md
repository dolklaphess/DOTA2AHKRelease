...
短时间内发送相反的滚轮指令可能会被系统block掉，AHK无法解决这一问题，安排快捷键时需要谨慎，同时由于dota渲染层堆叠次序的问题，全屏模式下有可能不能获取到正常的游戏颜色。

通常来说大多数脚本设定键均为Shift+S，重启键均为Ctrl+LWin。
...
新增UniversalTreads.ahk脚本，提供普适性的切假腿功能，在涉及到的装备不处于cd状态时按下Shift+S，可将当前假腿状态记录为默认主属性，此后可以自动在施法、攻击、移动前自动切换至对应的假腿（不同切换模式按Y调整），亦可用于最大化切臂章和使用魔棒的收益。完美适用于绝大多数4或6技能英雄，对5技能英雄不完美支持，不支持可切换开关的技能（如拉席克大招），不支持卡尔，水人，以及偷了上述技能的拉比克。但在未来计划推出水人的专门脚本。由于可能打断隐身，强烈建议对隐身英雄进行专门的细微修改后再使用。同时脚本内有按个人习惯的按B切换智力假腿用于应对技能差一点蓝放不出的情况，如不需求或键位冲突可自行修改。
