...
本脚本库中的按键部分按照作者个人按键习惯设置，其中大多在Config.ahk中修改，简单的AHK触发快捷键本身需要在对应脚本内修改。

作者个人使用统一键位快捷施法，如果使用普通施法的，可以单独设置快捷施法按键或在Send内加入{Click}

使用传统键位或者特定英雄有不同改键的，请按以下格式修改脚本自动运行段中加入快捷键数组：hero:=new DotaClass(n_ability,dir,,["q","w","e","r","d","f"],["q","w","e","r","d","f"])。基于假腿的可以在Config.ahk中加入变量global heroname_ability_triggerkey和global heroname_ability_keyup。
...
新增TrWeaver.ahk和TrSpectre.ahk，提供编织者与幽鬼的假腿相关改键。其中幽鬼在出A杖后界面有BUG，无法使用，需要等待V社修复。

在GeneralMapping.ahk中新增抽搐补刀功能，按住A键时按下S键将在取消当前攻击指令同时重新攻击，用于骗前摇补刀。

TrClinkz中也加入了类似功能，在按住火箭快捷键时按S将取消攻击并再次攻击。

## a0.70
2019-5-30 21:34:07

新增NyxAssassin.ahk，提供司夜刺客一键吹风接戳功能，默认的快捷键是Q。
...
新增GeneralMapping.ahk脚本，该脚本提供突破Dota2固有限制的改键设置（如按住鼠标中键时屏蔽滚轮以防止误触），更完善的切换自动攻击状态（需要将开启控制台快捷键设为小键盘del,可分别调整英雄和其余单位），同时提供持续高频率右键点击，一键交换背包和物品栏内道具等功能。使用本脚本的Win键映射为将英雄锁定于屏幕中心功能，需在dota2的autoexec.cfg文件中加入控制台指令：
