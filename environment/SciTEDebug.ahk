...
VL_Update()
{
	...
	VL_Static := Util_UnpackNodes(Dbg_StaticContext.selectNodes("/response/property/@name"))
	VL_Global := Util_UnpackNodes(Dbg_GlobalContext.selectNodes("/response/property/@name"))
	VL_NVars := VL_Local.MaxIndex() + VL_Static.MaxIndex() + VL_Global.MaxIndex()
	VL_LocalCont := Util_UnpackContNodes(Dbg_LocalContext.selectNodes("/response/property"))
	VL_StaticCont := Util_UnpackContNodes(Dbg_StaticContext.selectNodes("/response/property"))
	...
	Loop, % VL_Static.MaxIndex()
		LV_Add("", "Static", VL_Static[A_Index], VL_StaticCont[A_Index])
...
Dbg_GetContexts()
{
	...
	Dbg_Session.context_get("-c 1", Dbg_StaticContext)
	Dbg_Session.context_get("-c 2", Dbg_GlobalContext)
	Dbg_Session.feature_set("-n max_data -v " dbgMaxData)
	Dbg_LocalContext  := loadXML(Dbg_LocalContext)
	Dbg_StaticContext := loadXML(Dbg_StaticContext)
